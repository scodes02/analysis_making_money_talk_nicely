...
author: "Sam Hickman, Billy Geary, Matthew Cantele"
format:
  html: 
...
Reads in the necessary data and combines the data based on attributes.
...
# Reorder rows/columns to a consistent order (adjust to your actual tool names)
desired_order <- c("BIA_GBS", "FS", "GID", "GIST", "IDL", "MSCI", "NAlpha", "S&PS1")

cor.mat.agg <- cor.mat.agg[desired_order, desired_order]
cor.mat.va  <- cor.mat.va[desired_order, desired_order]

# Merge the two correlation matrices: top triangle = aggregate, bottom = value-adjusted
combined_mat <- cor.mat.agg
combined_mat[lower.tri(combined_mat)] <- cor.mat.va[lower.tri(combined_mat)]

# Blank out diagonal
diag(combined_mat) <- NA

# Convert to long format, rename BIA_GBS → BIA-GBS
df <- as.data.frame(combined_mat) |>
  ...
  mutate(
    ...
    # rename BIA_GBS
    Var1 = ifelse(Var1 == "BIA_GBS", "BIA-GBS", Var1),
    Var2 = ifelse(Var2 == "BIA_GBS", "BIA-GBS", Var2),
    # set factor levels
    Var1 = factor(Var1, levels = ifelse(desired_order == "BIA_GBS", "BIA-GBS", desired_order)),
    Var2 = factor(Var2, levels = ifelse(desired_order == "BIA_GBS", "BIA-GBS", desired_order))
  )
...
sectors_to_plot <- c("Consumer Staples", "Utilities")

# Helper to extract and label EMM results
extract_emm <- function(model, by_var, effect_name) {
...
emm_globio <- extract_emm(large_model_1, "GLOBIO",         "GLOBIO")
emm_lci    <- extract_emm(large_model_1, "LC_Impact",      "LC-Impact")
...
ft <- flextable(fixef_df) |>
  colformat_num(j = c("Mean","SD","2.5%","97.5%"), digits = 3) |>
  autofit()

doc <- read_docx() |>
  body_add_par("Posterior summaries for large_model_1", style = "heading 1") |>
  body_add_flextable(ft)

# print(doc, target = "regression_results_table/large_model_1_summary.docx")
```
...
num_health_above_median_energy <- sum(health_ranks > median_energy_rank)
...
```{r}
# Define tools
tools <- c("S&PS1", "MSCI", "IDL", "GID", "FS", "GIST", "NAlpha", "BIA_GBS")
...
ggsave(
  filename = "nsustainability_plots/figure_1.png",
...
ggsave(
  filename = "nsustainability_plots/figure_2.png",
...
ggsave(
  filename = "nsustainability_plots/figure_3.png",
...
ggsave(
  filename = "nsustainability_plots/figure_4.png",
  plot     = sector_plot,
  width    = 14,
  height   = 10,
  units    = "in",
...
# Supplementary Figure 1: Boxplot by tool facets
ggsave(
  filename = "nsustainability_plots/supplementary_figure_1.png",
  plot     = sector_box_all,
  width    = 14,
  height   = 10,
  units    = "in",
