...
# Set target options:
tar_option_set(
  packages = c("readxl", "data.table", "tibble", "tidyverse", "zoo", "haven","writexl"),
  format = "qs",
  garbage_collection = TRUE
)

# functions
functions <- list.files("targets_functions/R", pattern = "\\.R$", full.names = TRUE)
...
list(
  ...
  # S&P ------------------------------------------------------------------------
  tar_target(name = SP_raw_data,
             command = {data.table::fread(SP_file)}),
  tar_target(name = SP_aux_data,
             command = {data.table::setDT(readxl::read_xlsx(path = SP_isin))}),
  # merge ISIN and tickers for S&PS1
  tar_target(name = SP_merged_data,
             command = .merge_isin(data = SP_raw_data,
  ...
  tar_target(name = SP_wrangled_data,
             command = .wrangle_data(data = SP_merged_data,
  ...
  tar_target(name = unique_MSCI,
             command = {
               sort(colnames(MSCI_raw_data)[10:ncol(MSCI_raw_data)])
             }
  ),
  
  tar_target(name = MSCI_wrangled_data,
             command = .wrangle_data(data = MSCI_raw_data,
                                     tool = "MSCI",
  ...
  tar_target(name = MSCI_wrangled_data_va,
             command = .wrangle_data(data = MSCI_raw_data,
                                     tool = "MSCI",
  ...
  tar_target(name = IDL_wrangled_data,
             command = .wrangle_data(data = IDL_raw_data,
                                     tool = "IDL",
                                     endpoint = "CBF Value",
  ...
  tar_target(name = IDL_wrangled_data_va,
             command = .wrangle_data(data = IDL_raw_data,
                                     tool = "IDL",
  ...
  # BIA-GBS --------------------------------------------------------------------
  tar_target(name = BIA_GBS_raw_data,
             command = {
               data.table::setDT(readxl::read_xlsx(path = BIA_file,
                                                   sheet = "Entity Data"))
             }),
  
  tar_target(name = BIA_GBS_aux_data,
             command = {data.table::setDT(readxl::read_xlsx(path = BIA_file,
                                                            sheet = "Instrument Mapping"))}),
  
  # merge ISIN and tickers for S&PS1
  tar_target(name = BIA_GBS_merged_data,
             command = .merge_isin(data = BIA_GBS_raw_data,
  ...
  tar_target(name = unique_BIA_GBS,
             command = {
  ...
  tar_target(name = BIA_GBS_wrangled_data,
             command = .wrangle_data(data = BIA_GBS_merged_data,
                                     tool = "BIA_GBS",
  ...
  # FS -------------------------------------------------------------------------
  tar_target(name = FS_raw_data,
             command = {
               data.table::fread(FS_file)
             }),
  
  # unique endpoints here with description (use "tar_read(unique_GID)" to view)
  tar_target(name = unique_FS,
             command = {
               sort(colnames(FS_raw_data)[c(5,6)])
             }
  ),
  
  tar_target(name = FS_wrangled_data,
             command = .wrangle_data(ticker_data = SP500_company_data,
                                     data = FS_raw_data,
                                     tool = "FS",
                                     endpoint = "nSTAR",
                                     company_col = "Company",
  ...
  # GIST -----------------------------------------------------------------------
  tar_target(name = GIST_raw_data,
             command = {
               data.table::fread(GIST_file)
             }),
  
  tar_target(name = unique_GIST,
             command = {
               sort(colnames(GIST_raw_data)[c(9:ncol(GIST_raw_data))])
             }
  ),
  
  tar_target(name = GIST_requested,
             command = {
               readxl::read_xlsx(path = requested_tickers,
                                 sheet = "GIST")
             }),
  
  tar_target(name = GIST_wrangled_data,
             command = .wrangle_data(data = GIST_raw_data,
  ...
  # NAlpha -------------------------------------------------------------------------
  tar_target(name = NA_raw_data,
             command = {
               data.table::fread(NA_file)
             }),
  
  tar_target(name = unique_NA,
             command = {
               sort(colnames(NA_raw_data)[c(4:ncol(NA_raw_data))])
             }),
  
  tar_target(name = NA_wrangled_data,
             command = .wrangle_data(data = NA_raw_data,
                                     tool = "NAlpha",
  ...
  tar_target(name = NA_wrangled_data_va,
             command = .wrangle_data(data = NA_raw_data,
                                     tool = "NAlpha",
  ...
  tar_target(name = GICS_file,
             command = "data/finance_data/gics_industry_classification.xlsx",
             format = "file"),
  
  tar_target(name = GICS_data,
             command = {
               readxl::read_xlsx(path = GICS_file,
                                 skip = 3,
                                 sheet = 1)
             }),
  
  tar_target(name = SP500_company_file,
             command = "data/finance_data/SP500_companies.csv",
             format = "file"),
  
  tar_target(name = SP500_company_data,
             command = {
               read.csv(file = SP500_company_file)
             }),
  
  tar_target(name = SP500_isin_file,
             command = "data/finance_data/SNP500_ticker_isin.csv",
             format = "file"),
  
  tar_target(name = SP500_isin_data,
             command = {
               read.csv(file = SP500_isin_file)
             }),
  
  tar_target(name = revenue_file,
             command = "data/finance_data/revenue_sales_sp500.xlsx",
             format = "file"),
  
  tar_target(name = revenue_data,
             command = {
               readxl::read_xlsx(path = revenue_file)
             }),
  
  # Final steps ----------------------------------------------------------------
  tar_target(name = finance_dat,
             command = .finance_data(GICS_data = GICS_data,
  ...
  tar_target(name = va_supplemented_dat,
             command = .add_va(missing = c("S&PS1",
                                           "BIA_GBS",
                                           "FS",
                                           "GIST"),
  ...
  tar_target(name = f_dat_file,
             command = {
               fwrite(x = final_tool_dat,
